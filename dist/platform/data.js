window.NEWS_DATA = {
  "items": [
    {
      "title": "google.com/goto: Google's anti-scraping update",
      "link": "https://www.autom.dev/blog/google-search-goto-links",
      "desc": "Article URL: https://www.autom.dev/blog/google-search-goto-links Comments URL: https://news.ycombinator.com/item?id=49668386 Points: 508 # Comments: 404",
      "date": "2026-09-12",
      "source": "Hacker News",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "Elon Musk Has Pledged to Rid His Platform, X, of Child Sexual Abuse, but It Persists",
      "link": "https://www.nytimes.com/2026/09/11/technology/x-grok-child-images.html",
      "desc": "Reviews by the Canadian Center for Child Protection and The New York Times found that explicit images of children remain on the social media site owned by Elon Musk.",
      "date": "2026-09-11",
      "source": "NYT Tech",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "California’s Governor Signs Landmark Online Child Safety Bills",
      "link": "https://www.nytimes.com/2026/09/10/technology/californias-governor-gavin-newsom-online-child-safety-bills.html",
      "desc": "Gov. Gavin Newsom signed a bill that prohibits social media platforms from providing addictive features to users under 16, the first law of its kind in the nation.",
      "date": "2026-09-11",
      "source": "NYT Tech",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "MCP без хаоса: как мы создали в корпоративном ландшафте централизованный шлюз для ИИ‑агентов",
      "link": "https://habr.com/ru/companies/sberbank/articles/1080158/?utm_campaign=1080158&utm_source=habrahabr&utm_medium=rss",
      "desc": "Привет, Хабр! Меня зовут Анастасия Трегубова, я архитектор продукта Platform V Synapse API Mesh в СберТехе. Хочу поделиться идеями по работе с агентскими инструментами и внедрением протокола МСР в корпоративном ландшафте. Расскажу, как мы обеспечиваем централизацию, но обходим узкие места, и как ...",
      "date": "2026-09-11",
      "source": "Habr AI",
      "cat": "platform",
      "lang": "ru"
    },
    {
      "title": "Commit → Query → Test: разыгрываем консультацию",
      "link": "https://habr.com/ru/companies/usetech/news/1081174/?utm_campaign=1081174&utm_source=habrahabr&utm_medium=rss",
      "desc": "В сентябре отмечаем сразу три профессиональных праздника: 🎉 9 сентября — День тестировщика 🎉 13 сентября — День программиста 🎉 24 сентября — День системного аналитика В ГК «Юзтех» решили не размениваться на стандартные призы. Вместо мерча и сертификатов разыгрываем то, что действительно двигает к...",
      "date": "2026-09-11",
      "source": "Habr AI",
      "cat": "platform",
      "lang": "ru"
    },
    {
      "title": "Один фундамент для разных продуктов: как VK выстраивает платформенную разработку",
      "link": "https://tproger.ru/articles/odin-fundament-dlya-raznyh-produktov-kak-vk-vystraivaet-platform",
      "desc": "Как VK строит единую платформенную разработку для ВКонтакте, Одноклассников, Дзена, VK Видео и MAX: внутренняя платформа, One-cloud, видеоплатформа и OneAB. — Читать дальше « Один фундамент для разных продуктов: как VK выстраивает платформенную разработку »",
      "date": "2026-09-11",
      "source": "Tproger",
      "cat": "platform",
      "lang": "ru"
    },
    {
      "title": "Hugging Face выпустила TRL 1.13 с обучением на контексте свыше миллиона токенов",
      "link": "https://tproger.ru/news/hugging-face-vypustila-trl-1-13-s-obucheniem-na-kontekste-svywe-m",
      "desc": "TRL 1.13 ускоряет расчёт функции потерь и добавляет пример обучения на контексте свыше миллиона токенов. Из библиотеки удалён устаревший PPOTrainer. — Читать дальше « Hugging Face выпустила TRL 1.13 с обучением на контексте свыше миллиона токенов »",
      "date": "2026-09-11",
      "source": "Tproger",
      "cat": "platform",
      "lang": "ru"
    },
    {
      "title": "The EPA is planning to scrap public review rules for data center pollution",
      "link": "https://capitalbnews.org/data-centers-permit-rules-epa/",
      "desc": "Article URL: https://capitalbnews.org/data-centers-permit-rules-epa/ Comments URL: https://news.ycombinator.com/item?id=49662672 Points: 438 # Comments: 301",
      "date": "2026-09-11",
      "source": "Hacker News",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "650 млн точек, 3 поломки и PI System: технический разбор предиктивной аналитики на НПЗ (часть 2)",
      "link": "https://habr.com/ru/companies/datasapience/articles/1080892/?utm_campaign=1080892&utm_source=habrahabr&utm_medium=rss",
      "desc": "Часть 2 из 2. В первой части серии мы предсказали отказ насоса за 60 дней и выяснили, что годами чинили не то. Увидели, что материал встретил вашу живую реакцию, поэтому возвращаемся по горячим следам со второй частью. В этом материале, как и обещали, погружаемся в технику. Расскажем, что было по...",
      "date": "2026-09-10",
      "source": "Habr ML",
      "cat": "platform",
      "lang": "ru"
    },
    {
      "title": "Всероссийская олимпиада по ИИ впервые открыла участие для студентов колледжей",
      "link": "https://habr.com/ru/news/1080878/?utm_campaign=1080878&utm_source=habrahabr&utm_medium=rss",
      "desc": "Открылась регистрация на Всероссийскую олимпиаду по искусственному интеллекту. В этом году к участию впервые допустили студентов колледжей, техникумов и училищ. Ранее олимпиада была рассчитана на школьников 8–11 классов и иностранных учащихся. Зарегистрироваться можно до 6 октября на платформе Al...",
      "date": "2026-09-10",
      "source": "Habr AI",
      "cat": "platform",
      "lang": "ru"
    },
    {
      "title": "Selectel проведет юбилейную флагманскую конференцию Selectel ТехноДень 2026 в Москве",
      "link": "https://tproger.ru/news/selectel-provedet-yubilejnuyu-flagmanskuyu-konferenciyu-selectel-teh",
      "desc": "Selectel проведет юбилейную конференцию ТехноДень 2026 8 октября в Москве. Программа: ИИ, инфраструктура, безопасность, данные. Участие бесплатное. — Читать дальше « Selectel проведет юбилейную флагманскую конференцию Selectel ТехноДень 2026 в Москве »",
      "date": "2026-09-10",
      "source": "Tproger",
      "cat": "platform",
      "lang": "ru"
    },
    {
      "title": "IBM releases SOTA Granite Time Series PatchTST-FM-r2 model with commercial-friendly license",
      "link": "https://huggingface.co/blog/ibm-research/ibm-releases-sota-granite-time-series",
      "desc": "",
      "date": "2026-09-09",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "Suno представила v6 — новое поколение ИИ‑моделей для генерации музыки",
      "link": "https://habr.com/ru/companies/bothub/news/1080418/?utm_campaign=1080418&utm_source=habrahabr&utm_medium=rss",
      "desc": "Suno представила новое поколение моделей для генерации музыки — v6. В линейку вошли сразу три версии: флагманская v6, экспериментальная v6-wild и бесплатная v6-mini. По словам разработчиков, новые модели стали быстрее и лучше понимают структуру композиции, вокал, инструменты, настроение и музыкал...",
      "date": "2026-09-09",
      "source": "Habr AI",
      "cat": "platform",
      "lang": "ru"
    },
    {
      "title": "Safety for Whom? Refusing the Right Subset of a Topic, Not the Whole Topic",
      "link": "https://huggingface.co/blog/MultiverseComputingCAI/safety-for-whom",
      "desc": "",
      "date": "2026-09-08",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "Вышла FreeBSD 14.5 с поддержкой Linux API inotify",
      "link": "https://tproger.ru/news/vywla-freebsd-14-5-s-podderzhkoj-linux-api-inotify",
      "desc": "FreeBSD 14.5 получила нативный API inotify, LLVM 21.1.8 и изменения для Clang. Релиз доступен с 8 сентября, его поддержка продлится до 30 июня 2027 года. — Читать дальше « Вышла FreeBSD 14.5 с поддержкой Linux API inotify »",
      "date": "2026-09-08",
      "source": "Tproger",
      "cat": "platform",
      "lang": "ru"
    },
    {
      "title": "Пока все следят за Маском, Цукерберг вынужден менять алгоритмы? Соцсети проходят свой «момент Big Tobacco»?",
      "link": "https://habr.com/ru/articles/1079810/?utm_campaign=1079810&utm_source=habrahabr&utm_medium=rss",
      "desc": "Последние недели внимание индустрии приковано к X: 13 августа Илон Маск выложил в открытый доступ веса ранжирования алгоритма «For You» и запустил инструмент «Under the Hood», который прямо показывает пользователю, есть ли ограничения на видимость его постов и почему. Пока все обсуждают честен ли...",
      "date": "2026-09-08",
      "source": "Habr ML",
      "cat": "platform",
      "lang": "ru"
    },
    {
      "title": "Блогеры смогут управлять монетизацией контента на разных площадках из одного сервиса  —  Яндекс ПроБлогер",
      "link": "https://www.cossa.ru/news/350747/",
      "desc": "Яндекс объединил свои инструменты заработка на контенте в новый сервис Яндекс ПроБлогер. Авторы каналов и блогеры получат «одно окно» для рекламной монетизации и аналитики сразу всех ключевых контент-площадок — Telegram, МАКС, VK, RuTube, YouTube и Дзен. Аналогичных сервисов с таким широким набор...",
      "date": "2026-09-04",
      "source": "Cossa",
      "cat": "platform",
      "lang": "ru"
    },
    {
      "title": "NeoMME: an efficient Multimodal-native and Multilingual Encoder",
      "link": "https://huggingface.co/blog/Hcompany/neomme",
      "desc": "",
      "date": "2026-09-03",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "Fine-tuning a 350M Model for Better Structured Outputs in 100 GRPO Steps",
      "link": "https://huggingface.co/blog/grpo-with-trl-ifstruct",
      "desc": "",
      "date": "2026-09-03",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "Training a coding model to paint watercolours with TRL and OpenEnv",
      "link": "https://huggingface.co/blog/train-to-paint-with-code",
      "desc": "",
      "date": "2026-09-03",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "Introducing @huggingface/kernels: 200+ WebGPU Kernels for Local AI",
      "link": "https://huggingface.co/blog/webgpu-kernels",
      "desc": "",
      "date": "2026-09-01",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "The Open ASR Leaderboard Adds Its First Global South Language",
      "link": "https://huggingface.co/blog/open-asr-leaderboard-global-south",
      "desc": "",
      "date": "2026-08-28",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "Sharplink",
      "link": "https://www.awwwards.com/sites/sharplink",
      "desc": "Sharplink is the institutional-grade Ethereum treasury platform giving investors a smarter, more productive access vehicle to ETH.",
      "date": "2026-08-27",
      "source": "Awwwards",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "Quantization-Aware Healing: a compressed, 4-bit model that outperforms its full-precision original",
      "link": "https://huggingface.co/blog/MultiverseComputingCAI/quantization-aware-healing",
      "desc": "",
      "date": "2026-08-25",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "Wire It, Run It, Deploy It: AI Workflows in Gradio",
      "link": "https://huggingface.co/blog/gradio-workflow-guide",
      "desc": "",
      "date": "2026-08-25",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "How Hugging Face Inference Endpoints, Jobs, and Buckets Power Search on Papers with Code",
      "link": "https://huggingface.co/blog/pwc-search",
      "desc": "",
      "date": "2026-08-21",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "Measuring benchmark optimization in speech recognition",
      "link": "https://huggingface.co/blog/asr-benchmark-optimization",
      "desc": "",
      "date": "2026-08-21",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "Up to 3.2x Faster Inference with LFM2.5-DSpark",
      "link": "https://huggingface.co/blog/LiquidAI/lfm25-dspark",
      "desc": "",
      "date": "2026-08-20",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "Same Cluster, 33 Points More Utilization: What Changed Was the Order",
      "link": "https://huggingface.co/blog/Dharma-AI/gpu-management-pt2",
      "desc": "",
      "date": "2026-08-17",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "State of Open Models: Summer 2026 Observations",
      "link": "https://huggingface.co/blog/state-of-open-models-summer-2026",
      "desc": "",
      "date": "2026-08-14",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "What We Learned by Reproducing 2,200 papers from ICML",
      "link": "https://huggingface.co/blog/icml-2026-open-reproductions",
      "desc": "",
      "date": "2026-08-13",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "Introducing OlmoEarth embeddings: Custom embedding exports from OlmoEarth Studio for downstream analysis",
      "link": "https://huggingface.co/blog/allenai/olmoearth-embeddings",
      "desc": "",
      "date": "2026-08-12",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "Thinking of ACE? We Can Do It with Fewer Tokens",
      "link": "https://huggingface.co/blog/ibm-research/altk-evolve-sldd",
      "desc": "",
      "date": "2026-08-11",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "Making Knowledge Distillation Cheap Enough to Run at Scale",
      "link": "https://huggingface.co/blog/MultiverseComputingCAI/efficient-knowledge-distillation",
      "desc": "",
      "date": "2026-08-10",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "How Baseline Can Help You Ship Less JavaScript",
      "link": "https://smashingmagazine.com/2026/08/how-baseline-can-help-ship-less-javascript/",
      "desc": "The gap between “you need a library for this” and “the browser does this” keeps closing. A practical guide to auditing your dependencies and finding what the web platform can now handle for you.",
      "date": "2026-08-07",
      "source": "Smashing Magazine",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "Baseten on Hugging Face Inference Providers 🔥",
      "link": "https://huggingface.co/blog/baseten",
      "desc": "",
      "date": "2026-08-06",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "GPU Management: Why Idle GPUs Are the New Grounded Aircraft",
      "link": "https://huggingface.co/blog/Dharma-AI/gpu-management",
      "desc": "",
      "date": "2026-07-30",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "NVIDIA Cosmos-H-Dreams: Bringing Real-Time Generative Simulation to Surgical Robotics",
      "link": "https://huggingface.co/blog/nvidia/cosmos-h-dreams",
      "desc": "",
      "date": "2026-07-27",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "Grabette: an open system to record robot-manipulation data",
      "link": "https://huggingface.co/blog/grabette",
      "desc": "",
      "date": "2026-07-21",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "Newer Models, Same Advantage",
      "link": "https://huggingface.co/blog/Dharma-AI/newer-models-same-advantages",
      "desc": "",
      "date": "2026-07-16",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "Security incident disclosure — July 2026",
      "link": "https://huggingface.co/blog/security-incident-july-2026",
      "desc": "",
      "date": "2026-07-16",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "Model Routing Is Simple. Until It Isn’t.",
      "link": "https://huggingface.co/blog/ibm-research/model-routing-is-simple-until-it-isnt",
      "desc": "",
      "date": "2026-07-15",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "Welcome Inkling by Thinking Machines",
      "link": "https://huggingface.co/blog/thinkingmachines-inkling",
      "desc": "",
      "date": "2026-07-15",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "Introducing Real World VoiceEQ: Measuring the human quality of voice AI",
      "link": "https://huggingface.co/blog/real-world-voiceeq",
      "desc": "",
      "date": "2026-07-15",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "Profiling in PyTorch (Part 3): Attention is all you profile",
      "link": "https://huggingface.co/blog/torch-attention-profile",
      "desc": "",
      "date": "2026-07-10",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "Designing For Distressed Users: Why Mental Health Apps Shouldn’t Follow Every UI Fashion",
      "link": "https://smashingmagazine.com/2026/07/designing-distressed-users-mental-health-apps-ui/",
      "desc": "Many UI trends are designed to capture attention and signal innovation, but those goals often conflict with the needs of mental health apps: reducing cognitive strain, fostering trust, and providing a sense of refuge. Kat Homan introduces an evaluation framework that helps designers assess whethe...",
      "date": "2026-07-09",
      "source": "Smashing Magazine",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "From Hugging Face to Amazon SageMaker Studio in one click",
      "link": "https://huggingface.co/blog/amazon/one-click-to-sagemaker-studio",
      "desc": "",
      "date": "2026-07-07",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "Hugging Face Models on Foundry Managed Compute",
      "link": "https://huggingface.co/blog/microsoft/foundry-managed-compute",
      "desc": "",
      "date": "2026-07-07",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "Run AI workloads on any cloud, store on Hugging Face: zero-egress storage with SkyPilot",
      "link": "https://huggingface.co/blog/skypilot-hf-storage",
      "desc": "",
      "date": "2026-07-07",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "LeRobot v0.6.0: Imagine, Evaluate, Improve",
      "link": "https://huggingface.co/blog/lerobot-release-v060",
      "desc": "",
      "date": "2026-07-07",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "PRX Part 4: Our Data Strategy",
      "link": "https://huggingface.co/blog/Photoroom/prx-part4-data",
      "desc": "",
      "date": "2026-07-06",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "🤗 Kernels: Major Updates",
      "link": "https://huggingface.co/blog/revamped-kernels",
      "desc": "",
      "date": "2026-07-06",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "Hugging Face and Cerebras bring Gemma 4 to real-time voice AI",
      "link": "https://huggingface.co/blog/cerebras-gemma4-voice-ai",
      "desc": "",
      "date": "2026-07-01",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "Why Specialization Is Inevitable",
      "link": "https://huggingface.co/blog/Dharma-AI/why-specialization-is-inevitable",
      "desc": "",
      "date": "2026-06-30",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "Featuring Every Eval Ever Results on Hugging Face Model Pages",
      "link": "https://huggingface.co/blog/eee-community-evals",
      "desc": "",
      "date": "2026-06-30",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "Introducing the FFASR Leaderboard: Benchmarking ASR in the Real World",
      "link": "https://huggingface.co/blog/ffasr-leaderboard",
      "desc": "",
      "date": "2026-06-24",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "Shipping huggingface_hub every week with AI, open tools, and a human in the loop",
      "link": "https://huggingface.co/blog/huggingface-hub-release-ci",
      "desc": "",
      "date": "2026-06-23",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "Как я сделал генератор палитр на Node.js + DeepSeek за два вечера - и что из этого вышло",
      "link": "https://habr.com/ru/articles/1050734/?utm_campaign=1050734&utm_source=habrahabr&utm_medium=rss",
      "desc": "Как-то вечером я поймал себя на том, что трачу по 20 минут на поиск цветовой палитры для каждого нового проекта. Coolors, Adobe Color, случайные пины в Pinterest — и всё равно ощущение «не то». Тогда я задал себе вопрос: а что если вместо колеса оттенков начинать со слова ? «Рассвет», «шторм», «л...",
      "date": "2026-06-23",
      "source": "Habr Веб-дизайн",
      "cat": "platform",
      "lang": "ru"
    },
    {
      "title": "PP-OCRv6 on Hugging Face: 50-Language OCR from 1.5M to 34.5M Parameters",
      "link": "https://huggingface.co/blog/PaddlePaddle/pp-ocrv6",
      "desc": "",
      "date": "2026-06-22",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "We got local models to triage the OpenClaw repo for FREE!*",
      "link": "https://huggingface.co/blog/local-models-pr-triage",
      "desc": "",
      "date": "2026-06-22",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "Beyond LoRA: Can you beat the most popular fine-tuning technique?",
      "link": "https://huggingface.co/blog/peft-beyond-lora",
      "desc": "",
      "date": "2026-06-18",
      "source": "Hugging Face",
      "cat": "platform",
      "lang": "en"
    },
    {
      "title": "Эволюция токенов цветов в Туту: как навести порядок в дизайне и не уехать в дурку",
      "link": "https://habr.com/ru/articles/1048072/?utm_campaign=1048072&utm_source=habrahabr&utm_medium=rss",
      "desc": "Создание единой дизайн-системы для крупного кроссплатформенного продукта — это всегда вызов. А если процесс совпадает с масштабным ребрендингом компании, задача усложняется в разы. В этой статье мы разберем двухлетний кейс команды «Туту», которая прошла путь от полного отсутствия стандартов до ги...",
      "date": "2026-06-16",
      "source": "Habr Веб-дизайн",
      "cat": "platform",
      "lang": "ru"
    }
  ],
  "cat_keys": [
    "ai",
    "vibe",
    "agent",
    "platform",
    "design",
    "jobs",
    "orders"
  ],
  "updated": "2026-09-12"
};
